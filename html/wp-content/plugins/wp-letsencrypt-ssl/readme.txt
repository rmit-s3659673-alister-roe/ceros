...
Tags: ssl,ssl certificate,free ssl,https,force ssl,mixed content,lets encrypt,force https,insecure content,https redirect,padlock,enable ssl
...
Generate free Let's Encrypt SSL certificate for your WordPress site in One Click and force SSL, fixing insecure content & mixed content issues easily.
...
== 100000+ Downloads -- 40000+ SSL certificates generated with one click -- 3500+ PRO users ==

https://youtu.be/KQ2HYtplPEk

= CHECK OUT OUR OTHER PLUGINS =

[Modern Addons for Elementor](https://modernelementor.com/?utm_campaign=modernaddons&utm_medium=wordpress&utm_source=wpencryption) - Premium unique CSS3 based modern addon elements for Elementor page builder for free.

[Go Viral Social Plugin](https://wordpress.org/plugins/go-viral/) - A Complete social toolkit with social share, sticky sharebar, whatsapp chat, share & view counters, open graph, social locker, voting and much more.

= REQUIREMENTS =
PHP 5.4 & above, Linux hosting, OpenSSL, cPanel or SSH, CURL, allow_url_fopen should be enabled. More details on [WPEncryption.com](https://wpencryption.com)
...
1. Flawless One click SSL installation for cPanel sites without the need of ANY login

2. Wildcard SSL support

3. Multisite + Mapped domains support

4. Automatic DNS based domain verification for Wildcard SSL installation (DNS should be managed by cPanel or Godaddy).

5. Auto renewal of SSL certificate every 90 days before the expiry date.

6. Automated DNS verification in case of HTTP verification fail (DNS should be managed by cPanel or Godaddy).

7. Top notch one to one priority support (We will setup SSL for your site one or the other way)

8. One time free manual setup for sites with NO cPanel (requires Linux based Apache/Nginx server with root shell access).
...
Some helpful tutorials for non cPanel based SSL installations can be found on our [BLOG](https://gowebsmarty.com/blog/?utm_campaign=wpencryption&utm_source=wordpress&utm_medium=faq).
...
= Images not loading on HTTPS site =
Images on your site might be loading over http:// protocol, please enable "Force HTTPS" feature via WP Encryption page. If you have Elementor page builder installed, please go to Elementor > Tools > Replace URL and replace your http:// site url with https://. Make sure you have SSL certificates installed and browser padlock shows certificate as valid before forcing these https measures.
...
VirtualHost for port 443 (in case of apache server), Server{} listening at port 443 (in case of nginx server) might be not defined or correct SSL paths might be not set.
