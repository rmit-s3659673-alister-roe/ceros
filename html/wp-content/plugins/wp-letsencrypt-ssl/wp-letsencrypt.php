...
///define('WPLE_TX', 'wp-letsencrypt-ssl');
...
if ( function_exists( 'wple_fs' ) && !function_exists( 'wple_fs_custom_connect_message' ) ) {
    function wple_fs_custom_connect_message( $message )
    {
        ...
        return sprintf( esc_html__( 'Howdy %1$s' ) . ',<br>' . __( 'We <b>HIGHLY</b> recommend to Opt-in to get BEST support!. Never miss an important update - opt in to our security & feature updates notifications, and non-sensitive diagnostic tracking with <a href="https://freemius.com/wordpress/usage-tracking/5090/wp-letsencrypt-ssl/" target="_blank">Freemius</a>. If you skip this, that\'s okay! <b>WP Encryption</b> will still work just fine.', 'wp-letsencrypt-ssl' ), ucfirst( $current_user->user_nicename ) );
    }
